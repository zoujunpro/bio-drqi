...
SERVICE_DIR="/Users/zoujun/data/service/bio-cer-dev"
COMPOSE_FILE="${SERVICE_DIR}/docker-compose.yml"
IMAGE_NAME="bio-cer-service:dev"
BASE_IMAGE="bio-cer-service:dev-base"
LOCAL_JAVA_BASE_IMAGE="nacos/nacos-server:v2.3.2"
CONTAINER_NAME="bio-cer-service-dev"
...
services:
  bio-cer-service-dev:
    image: bio-cer-service:dev
    container_name: bio-cer-service-dev
    environment:
      - NACOS_HOST=nacos-standalone
      - DISCOVERY_HOST=bio-cer-service-dev
      - NACOS_PORT=8848
      - SPIRNG_PROFILES_ACTIVE=dev
      - SERVER_PORT=18093
      - DEBUG_PORT=28093
      - JAVA_OPTS= -Xmx1g -Xms1g -Xmn512m -Xss256k
      - TZ=Asia/Shanghai
    restart: always
    volumes:
      - "./logs:/app/logs"
...
echo "Building bio-drqi-admin jar..."
mvn -pl bio-drqi-admin -am clean package -DskipTests
...
docker build --platform linux/amd64 --build-arg BASE_IMAGE="${BASE_IMAGE}" -f bio-drqi-admin/Dockerfile.dev-local -t "${IMAGE_NAME}" bio-drqi-admin
...
cd "${SERVICE_DIR}"
docker compose -f "${COMPOSE_FILE}" up -d --force-recreate bio-cer-service-dev
...
echo "Done. bio-cer-service-dev is deployed."
trap - EXIT
